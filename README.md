...
[`src/lib/brand/bealls-family-runtime-contract.ts`](src/lib/brand/bealls-family-runtime-contract.ts) inventories all 30 executable route endpoints and all 28 named zone IDs. It distinguishes available, input-gated, and not-applicable routes; mounted, declared-only, and not-applicable zones; shopper, merchant-review, operator, development, and API audiences; and desktop/mobile regression cells. Home Centric's content category stays distinct from storefront PLP. Its direct storefront-only routes and APIs fail before catalog, cart, or model work begins.
...
Run `npm run test:contracts` to check both contracts. The runtime test scans every executable route and covers the full brand-zone desktop/mobile matrix. The renderer test scans direct `LayoutRenderer` use and recomputes SHA-256 fingerprints from recorded route, component, CSS, and config owners. These are pinned internal regression gates. They do not establish parity with an external storefront, and they leave every `reference.state` value `uncontracted`.
