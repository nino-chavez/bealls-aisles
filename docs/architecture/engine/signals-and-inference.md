...
The Aisles persona inference system is a continuous signal stream → probability vector pipeline. It reads signals from two sources (server-side request data and client-side behavioral events), evaluates 27 weighted rules against the accumulated context, and produces a `PersonaInference` object for deterministic catalog ranking and operator inspection.

Shopper layout generation is retired. The current Bealls-family runtime resolves named zones through fixed fallbacks or the registered `pdp-tag-overlap-v1` rule. The prompt material below documents the superseded whole-layout prototype; it is not active shopper behavior or authority.
...
## Historical: How Inference Fed Whole-Layout Generation

This section records the retired prototype contract. `buildLayoutPrompt()` remains for offline or explicitly authorized operator tooling. Shopper routes do not call it, and `/api/layout` rejects before input, cache, catalog, or provider work.
...
In the retired prototype, this allowed the model to blend layout styles for ambiguous sessions. It does not describe current shopper rendering.
...
- `src/lib/server/layout-prompt.ts` — retained historical/offline prompt builder; not a shopper runtime path
