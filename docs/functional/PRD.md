...
| **PRD-FND-010** | Real checkout via BC Optimized One-Page handoff | H2, H3 | planned (Phase 1) | Cart CTA hands off to BC Optimized Checkout. Order flows to merchant's existing BC fulfillment. No custom payment integration. |
...
| **PRD-FND-012** | Branded 404 / empty-state pages with engine rescue | H1, H2 | planned (Phase 1) | 404 pages render brand chrome + AI-composed rescue (alt CTAs, related categories, popular products). Empty cart, empty search, empty wishlist all share this pattern. |
| **PRD-FND-013** | Section authoring model — named insertion zones | H2 | planned (Phase 2) | Foundation defines named zones across all surfaces: `home.hero`, `home.featured-row.{n}`, `plp.banner`, `pdp.related`, `pdp.recently-viewed`, `cart.cross-sell`, `search.empty-state`, `account.welcome`. Engine composes into these zones; admin authors content for them. |
